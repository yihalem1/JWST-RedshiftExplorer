...
from tools.pseudo_NTIRE_data import NTIRE
...
def main(rank, world_size, cpu=False):
    ...
    trainset = NTIRE(data_lr=os.path.join(CFG.DATA.FOLDER, "train_32"), data_hr=os.path.join(CFG.DATA.FOLDER, "High_128"), img_range=CFG.DATA.IMG_RANGE, rgb=CFG.DATA.RGB)
    loader = get_train_loader(trainset, world_size, batch_per_gpu)
    testset = NTIRE(data_lr=os.path.join(CFG.DATA.FOLDER, "val_32"), data_hr=None, b_train=False, shuffle=False, img_range=CFG.DATA.IMG_RANGE, rgb=CFG.DATA.RGB)

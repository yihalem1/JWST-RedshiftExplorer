...
class GalaxyZoo_test(Dataset):
    def __init__(self, data_lr, data_hr=None, gt_lr=None, rgb=True, img_range=1, z_size=(3, 3)):
        print("lr path: ", data_lr)
        print("hr path: ", data_hr)
        
        self.ready_hr = data_hr is not None
        self.ready_gt_lr = data_hr is not None
        if data_hr is not None:
            self.hr_files = glob.glob(os.path.join(data_hr, "**/*.png"), recursive=True)
            self.hr_files.sort()

        if gt_lr is not None:
            self.gt_lr_files = glob.glob(os.path.join(gt_lr, "**/*.png"), recursive=True)
            self.gt_lr_files.sort()
    ...
    def __getitem__(self, index):
        ...
        lr = cv2.imread(self.lr_files[lr_idx], cv2.IMREAD_GRAYSCALE)#np.load(self.lr_files[lr_idx])
        ...
        if self.ready_hr:
            ...
            hr = cv2.imread(self.hr_files[hr_idx], cv2.IMREAD_GRAYSCALE)#np.load(self.hr_files[hr_idx])

            data["hr"] = self.preproc(hr) * self.img_min_max[1]
            data["hr_path"] = self.hr_files[hr_idx]
            data["hr_down"] = nnF.interpolate(data["hr"].unsqueeze(0), scale_factor=0.25, mode="bicubic", align_corners=False, recompute_scale_factor=False).clamp(min=self.img_min_max[0], max=self.img_min_max[1]).squeeze(0)
        
        if self.ready_gt_lr:
            gt_lr_idx = index % len(self.gt_lr_files)
            gt_lr = cv2.imread(self.gt_lr_files[gt_lr_idx], cv2.IMREAD_GRAYSCALE)#np.load(self.hr_files[hr_idx])

            data["gt_lr"] = self.preproc(gt_lr) * self.img_min_max[1]
...
class GalaxyZoo(Dataset):
    def __init__(self, data_lr, data_hr=None, b_train=True, rgb=True, img_range=1, shuffle=True, z_size=(3, 3)):
        print("lr path: ", data_lr)
        print("hr path: ", data_hr)
    ...
    def __getitem__(self, index):
        ...
        lr = self.preproc(lr)
        lr = (lr - lr.min()) / (lr.max() - lr.min())
        data["lr_original"] = lr * self.img_min_max[1]
        data["lr_path"] = self.lr_files[lr_idx]
        data["lr"] = nnF.interpolate(data["lr_original"].unsqueeze(0), scale_factor=1, mode="bicubic", align_corners=False).clamp(min=self.img_min_max[0], max=self.img_min_max[1]).squeeze(0)
...
if __name__ == "__main__":
    ...
    high_folder = "/home/s20225004/pseudo-sr/GalaxyZoo/48X48/Resized"
    low_folder = "/home/s20225004/pseudo-sr/GalaxyZoo/12X12/Noised"
    test_folder = "/home/s20225004/pseudo-sr/GalaxyZoo/val12x12"

    img_range = 1
    data = GalaxyZoo(low_folder, high_folder, img_range=img_range)
    print(len(data))
    for i in range(len(data)):
        ...
        #     cv2.imwrite("{}_{}.png".format(i, elem), img[:, :, ::-1])
        # cv2.waitKey()
    print("fin.")

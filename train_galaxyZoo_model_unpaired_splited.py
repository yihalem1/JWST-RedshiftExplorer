...
def main(rank, world_size, cpu=False):
    ...
    trainset = GalaxyZoo(data_lr=os.path.join(CFG.DATA.FOLDER, "weak_lensing/20220814/weak_12x12_train"), data_hr=os.path.join(CFG.DATA.FOLDER, "GalaxyZoo/48X48/Resized"), img_range=CFG.DATA.IMG_RANGE)
    loader = get_train_loader(trainset, world_size, batch_per_gpu)
    testset = GalaxyZoo(data_lr=os.path.join(CFG.DATA.FOLDER, "weak_lensing/20220814/weak_12x12_test"), data_hr=os.path.join(CFG.DATA.FOLDER, "GalaxyZoo/48X48/val_Resized_48X48"), b_train=False, shuffle=False, img_range=CFG.DATA.IMG_RANGE)

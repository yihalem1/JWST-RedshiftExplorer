...
class Galaxy_Model(Pseudo_Model):
    def __init__(self, device, cfg, use_ddp=False):
        super(Galaxy_Model, self).__init__(device=device, cfg=cfg, use_ddp=use_ddp)
        ...
        if self.use_esrgan:
            ...
            self.U = RRDBNet(cfg.DATA.CHANNEL, cfg.DATA.CHANNEL, scale_factor=cfg.SR.SCALE).to(device)
            self.D_esrgan = NLayerDiscriminator(cfg.DATA.CHANNEL, scale_factor=1, norm_layer=nn.InstanceNorm2d).to(device)
            if use_ddp:

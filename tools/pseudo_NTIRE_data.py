...
class NTIRE(Dataset):
    def __init__(self, data_lr, data_hr=None, b_train=True, rgb=True, img_range=1, shuffle=True, z_size=(16, 16)):
        ...
        if data_hr is not None:
            self.hr_files = glob.glob(os.path.join(data_hr, "**/*.JPEG"), recursive=True)
            self.hr_files.sort()
        self.lr_files = glob.glob(os.path.join(data_lr, "**/*.JPEG"), recursive=True)
        self.lr_files.sort()
        if shuffle:
            if data_hr is not None:
    ...
    def __getitem__(self, index):
        ...
        lr = cv2.imread(self.lr_files[lr_idx], cv2.IMREAD_GRAYSCALE)
        if self.rgb:
        ...
        if self.ready_hr:
            ...
            hr = cv2.imread(self.hr_files[hr_idx], cv2.IMREAD_GRAYSCALE)
            if self.rgb:
...
if __name__ == "__main__":
    ...
    high_folder = "/home/s20225004/pseudo-sr/imagenet_dataset/High"
    low_folder = "/home/s20225004/pseudo-sr/imagenet_dataset/train"
    test_folder = "/home/s20225004/pseudo-sr/imagenet_dataset/val"

    img_range = 1
    data = NTIRE(low_folder, high_folder, img_range=img_range)
    print(len(data))

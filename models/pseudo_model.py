...
class Pseudo_Model():
    def __init__(self, device, cfg, use_ddp=False):
        ...
        self.U = make_SR_net(rgb_range=rgb_range, rgb_mean=rgb_mean_point, scale_factor=cfg.SR.SCALE, in_chan=self.in_chan).to(device)
    ...
    def net_save(self, folder, shout=False):
        ...
        #     if isinstance(self.nets[k], nn.DataParallel):
        #         nets[k] = v.module.state_dict()
        #     else:
        #         nets[k] = v.state_dict()
        # #####
    ...
    def train_step(self, Ys, Xs, Yds, Zs):
        ...
        Zs: noises
        '''
        # print("Hr: {}, Lr: {}, Dwon Hr: {}".format(Ys.shape, Xs.shape, Yds.shape))
        # print("Hr: {}, Lr: {}, Dwon Hr: {}, Noise: {}".format(Ys.shape, Xs.shape, Yds.shape, Zs.shape))
        self.n_iter += 1
        loss_dict = dict()

        # forward
        fake_Xs = self.G_yx(Yds, Zs) # Downsampled High Resolution -> Fake Low Resoltion
        rec_Yds = self.G_xy(fake_Xs) # Fake Low Resolution -> Reconstructed Downsampled High Resolution
        fake_Yds = self.G_xy(Xs) # Low Resolution -> Fake Downsampled High Resolution
...
if __name__ == "__main__":
    ...
    with open("configs/WeakLensing.yaml", "rb") as cf:
    ...
    yd = torch.randn(8, CFG.DATA.CHANNEL, 24, 24, dtype=torch.float32, device=device)

...
from tools.pseudo_Galaxy_data import WeakLensing
...
def main(rank, world_size, cpu=False):
    ...
    trainset = WeakLensing(data_lr=os.path.join(CFG.DATA.FOLDER, "only_12x12"), data_hr=os.path.join(CFG.DATA.FOLDER, "EFIGI_r_cropped_as_48x48"), img_range=CFG.DATA.IMG_RANGE)
    loader = get_train_loader(trainset, world_size, batch_per_gpu)
    testset = WeakLensing(data_lr=os.path.join(CFG.DATA.FOLDER, "val_12x12"), data_hr=None, b_train=False, shuffle=False, img_range=CFG.DATA.IMG_RANGE)
    ...
    for ep in range(1, end_ep):
        ...
        if ep % 1 == 0 and rank == last_device:
            ...
            for b in range(len(testset)):
                ...
                save_tensor_image(os.path.join(img_save_folder, f"{b:04d}_y.png"), y, CFG.DATA.IMG_RANGE, CFG.DATA.RGB, CFG.DATA.CHANNEL)
                save_tensor_image(os.path.join(img_save_folder, f"{b:04d}_sr.png"), sr, CFG.DATA.IMG_RANGE, CFG.DATA.RGB, CFG.DATA.CHANNEL)
                save_tensor_image(os.path.join(img_save_folder, f"{b:04d}_lr.png"), lr, CFG.DATA.IMG_RANGE, CFG.DATA.RGB, CFG.DATA.CHANNEL)

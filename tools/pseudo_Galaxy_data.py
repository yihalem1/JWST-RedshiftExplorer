...
class WeakLensing(Dataset):
    def __init__(self, data_lr, data_hr=None, b_train=True, rgb=True, img_range=1, shuffle=True, z_size=(3, 3)):
        ...
        if data_hr is not None:
            self.hr_files = glob.glob(os.path.join(data_hr, "**/*.npy"), recursive=True)
            self.hr_files.sort()
        self.lr_files = glob.glob(os.path.join(data_lr, "**/*.npy"), recursive=True)
        self.lr_files.sort()
        if shuffle:
            if data_hr is not None:
    ...
    def __getitem__(self, index):
        ...
        lr = np.load(self.lr_files[lr_idx])
        ...
        if self.ready_hr:
            ...
            hr = np.load(self.hr_files[hr_idx])
...
if __name__ == "__main__":
    ...
    high_folder = "/home/s20225004/pseudo-sr/weak_lensing/EFIGI_r_cropped_as_48x48"
    low_folder = "/home/s20225004/pseudo-sr/weak_lensing/only_12x12"
    test_folder = "/home/s20225004/pseudo-sr/weak_lensing/val_12x12"

    img_range = 1
    data = WeakLensing(low_folder, high_folder, img_range=img_range)
    print(len(data))
